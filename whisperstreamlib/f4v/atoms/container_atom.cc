...
bool ContainerAtom::EqualsBody(const BaseAtom& other) const {
  const ContainerAtom& a = static_cast<const ContainerAtom&>(other);
  return AllEqualsP(subatoms_, a.subatoms_);
}
